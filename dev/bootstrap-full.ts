...
async function main() {
  ...
  // ── 1. Delete existing user (cascades all data) and recreate ───────────────
  console.log(`\n→ Looking up user: ${USER_EMAIL}`);

  const { data: userList } = await adminClient.auth.admin.listUsers();
  const existingUser = userList?.users.find((u) => u.email === USER_EMAIL);
  ...
  if (existingUser) {
    ...
    console.log("  Deleted user and all associated data (cascade)");
  }
